...
import './Gincana.css';
import timeVermelho from './img/timerosa.jpg';

const Timerosa = () => {
  // Recupera a pontuação inicial do localStorage ou usa 0 se não existir
  const [descpalav, setDescPalav] = useState(
    () => parseInt(localStorage.getItem('descpalav4')) || 0
  );
  const [cantmusi, setCantMusi] = useState(
    () => parseInt(localStorage.getItem('cantmusi4')) || 0
  );
  const [acertliv, setAcertLiv] = useState(
    () => parseInt(localStorage.getItem('acertliv4')) || 0
  );

  const [raclog, setRaclog] = useState(
    () => parseInt(localStorage.getItem('raclog4')) || 0
  );

  const [arrucor, setArrucor] = useState(
    () => parseInt(localStorage.getItem('arrucor4')) || 0
  );

  const [racha, setRacha] = useState(
    () => parseInt(localStorage.getItem('racha4')) || 0
  );

  const [ctorta, setTorta] = useState(
    () => parseInt(localStorage.getItem('ctorta4')) || 0
  );

  const [saiamuro, setSaiamuro] = useState(
    () => parseInt(localStorage.getItem('saiamuro4')) || 0
  );

  const [versiculo, setVersiculo] = useState(
    () => parseInt(localStorage.getItem('versiculo4')) || 0
  );

  const [tesouro, setTesouro] = useState(
    () => parseInt(localStorage.getItem('tesouro4')) || 0
  );

  const [circuito, setCircuito] = useState(
    () => parseInt(localStorage.getItem('circuito4')) || 0
  );

  const [bolbal, setBolbal] = useState(
    () => parseInt(localStorage.getItem('bolbal4')) || 0
  );

  const [enrosco, setEnrosco] = useState(
    () => parseInt(localStorage.getItem('enrosco4')) || 0
  );

  const [enche, setEnche] = useState(
    () => parseInt(localStorage.getItem('enche4')) || 0
  );

  const [velha, setVelha] = useState(
    () => parseInt(localStorage.getItem('velha4')) || 0
  );
  ...
  useEffect(() => {
    localStorage.setItem('descpalav4', descpalav);
  }, [descpalav]);

  useEffect(() => {
    localStorage.setItem('cantmusi4', cantmusi);
  }, [cantmusi]);

  useEffect(() => {
    localStorage.setItem('acertliv4', acertliv);
  }, [acertliv]);

  useEffect(() => {
    localStorage.setItem('raclog4', raclog);
  }, [raclog]);

  useEffect(() => {
    localStorage.setItem('arrucor4', arrucor);
  }, [arrucor]);


  useEffect(() => {
    localStorage.setItem('racha4', racha);
  }, [racha]);

  useEffect(() => {
    localStorage.setItem('ctorta4', ctorta);
  }, [ctorta]);

  useEffect(() => {
    localStorage.setItem('saiamuro4', saiamuro);
  }, [saiamuro]);

  useEffect(() => {
    localStorage.setItem('versiculo4', versiculo);
  }, [versiculo]);

  useEffect(() => {
    localStorage.setItem('tesouro4', tesouro);
  }, [tesouro]);

  useEffect(() => {
    localStorage.setItem('circuito4', circuito);
  }, [circuito]);

  useEffect(() => {
    localStorage.setItem('bolbal4', bolbal);
  }, [bolbal]);

  useEffect(() => {
    localStorage.setItem('enrosco4', enrosco);
  }, [enrosco]);

  useEffect(() => {
    localStorage.setItem('enche4', enche);
  }, [enche]);

  useEffect(() => {
    localStorage.setItem('velha4', velha);
  }, [velha]);
